...
Map LoadMap(const ObjJSON& map_object, extra_data::LootTypes& loot_types, double speed) {
...
Game LoadGame(const fs::path& json_path, extra_data::LootTypes& loot_types,  bool is_random_spawn) {
    ...
    game.SetSpeed(LoadSpeed(root.as_object()));
    
    for(const auto& map : maps_in_game->as_array()) {
        game.AddMap(LoadMap(map.as_object(), loot_types, game.GetSpeed()));
    }
