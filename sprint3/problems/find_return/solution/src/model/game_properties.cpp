...
const static double lost_objects_width = 0.0;
...
void GameSession::GenerateLoot(const std::chrono::milliseconds& delta) {
    ...
    for(size_t i = 0; i < new_objects_count; ++i) {
        lost_objects_.emplace_back(next_loot_id++, GenerateRandomLootType(), GenerateRandomPosition());
    }
}
...
void GameSession::ProcessLoot() {
    ...
    for (const auto& obj : lost_objects_) {
        ...
        items.emplace_back(geom::Point2D{pos.x, pos.y}, lost_objects_width/2.);
    }
