...
StringResponse ApiHandler::HanldeApiRequest(const StringRequest& req, targets_storage::TargetRequestType req_type) {
    using namespace targets_storage; 
    ...
    switch (req_type) {
        case TargetRequestType::GET_MAPS_INFO : {
            ...
            response.body() = json_constructor::MakeBodyJSON(req_type, game_);
            break;
        }
        ...
        case TargetRequestType::GET_MAP_BY_ID : {
            ...
            response.body() = json_constructor::MakeBodyJSON(req_type, game_, req_obj);
            break;
        }

        case TargetRequestType::POST_JOIN_GAME : {
            auto joining_info = json_loader::LoadJoiningInfo(req.body().data());
            if(!joining_info) {
                status = http::status::bad_request;
                response.body() = json_constructor::MakeBodyErrorJSON(TargetErrorCode::ERROR_INVALID_ARGUMENT_CODE,
                ...
                response.body() = json_constructor::MakeBodyErrorJSON(TargetErrorCode::ERROR_SEARCH_MAP_CODE,
                                                                      TargetErrorMessage::ERROR_SEARCH_MAP_MESSAGE,
                                                                      *joining_info->map_id);
            } else {
                player::AuthorizationInfo info = ProcessJoinGame(*joining_info);
                status = http::status::ok;
                response.body() = json_constructor::MakeBodyJSON(req_type, info);
            }
        ...
        case TargetRequestType::GET_PLAYERS : {
            auto field_iter = req.find("Authorization");
            if(field_iter == req.end()) {
                status = http::status::unauthorized;
                response.body() = json_constructor::MakeBodyErrorJSON(TargetErrorCode::ERROR_INVALID_TOKEN_CODE,
            ...
            if(!token) {
                ...
                response.body() = json_constructor::MakeBodyErrorJSON(TargetErrorCode::ERROR_INVALID_TOKEN_CODE,
                                                                      TargetErrorMessage::ERROR_INVALID_TOKEN_MESSAGE);
            } else if(auto player = players_.FindPlayerByToken(player::Token(*token))) {
                const auto session = player.value()->GetGameSession();
                const auto dogs = session.GetDogs();

                status = http::status::ok;
                response.body() = json_constructor::MakeBodyJSON(req_type, dogs);       
            } else {
                status = http::status::unauthorized;
                response.body() = json_constructor::MakeBodyErrorJSON(TargetErrorCode::ERROR_SEARCH_TOKEN_CODE,
        ...
        default : {
            response.body() = json_constructor::MakeBodyJSON(req_type);
            break;
        }
    }
...
std::string ApiHandler::ComputeRequestedObject(string_view target) {
...
player::AuthorizationInfo ApiHandler::ProcessJoinGame(const player::JoiningInfo& info) {
    model::UnitParameters parameters = game_.PrepareUnitParameters(info.map_id, info.user_name);
...
StringResponse RequestHandler::HandleErrorRequest(const StringRequest& req, TargetRequestType req_type) {
    ...
    switch (req_type) {
        ...
        default: {
            auto message = json_constructor::MakeBodyErrorJSON(TargetErrorCode::ERROR_INVALID_METHOD_CODE,
            ...
            response = MakeStringOtherResponse(status, req, message, content_type,
                                               allowed_methods.find(req.target())->second);
            break;
        }
    }
...
StringResponse RequestHandler::MakeStringOtherResponse(http::status status, const StringRequest& req, string_view message, 
                                                       string_view content_type, string_view allowed_methods) {
    ...
    if(!allowed_methods.empty()) {
        response.insert(http::field::allow, allowed_methods),
        response.insert(http::field::cache_control, "no-cache");
    }
