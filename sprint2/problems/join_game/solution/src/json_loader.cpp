...
ValueJSON ParseJSON(const std::string& value) {
...
std::optional<player::JoiningInfo> LoadJoiningInfo(const std::string& req_post) {
    try {
        ...
        std::string user_name = std::string(FindKey(value.as_object(), json_tag::USER_NAME)->as_string());  
        std::string map_id = std::string(FindKey(value.as_object(), json_tag::MAP_ID)->as_string());
