...
string MakeBodyJSON(TargetRequestType req_type, const player::AuthorizationInfo& object) {
    ...
    switch (req_type) {
        case TargetRequestType::POST_JOIN_GAME : 
            ...
            result[json_tag::AUTH_TOKEN] = *object.token;
            result[json_tag::PLAYER_ID] = object.player_id;
            break;
...
string MakeBodyJSON(TargetRequestType req_type, const std::vector<model::Dog>& dogs) {
    json::object result;
    
    for(const auto dog_it : dogs){
        boost::json::object dog;
