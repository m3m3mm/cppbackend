...
ResponseInfo Application::MoveUnit(const Header& header, const std::string& req_body) {
    ...
    return ExecuteAuthorized(header, [&req_body, this](const string& token) {
        if(auto player = players_->FindPlayerByToken(Token(token))) {
            if(auto dir = json_loader::LoadMoveInfo(req_body)) {
                ProcessMoveUnit(*player.value(), *dir);
            } else {
                return ResponseInfo {http::status::bad_request,
                                     MakeBodyErrorJSON(TargetErrorCode::ERROR_INVALID_ARGUMENT_CODE,
                                                     TargetErrorMessage::ERROR_FAILED_PARSE_MESSAGE)};
            }
...
void Application::ProcessMoveUnit(const Player& player, Direction dir) {
    auto dog = player.GetDog();
    auto map = player.GetGameSession().GetMap();
    auto speed = map.GetSpeed();
    switch (dir) {
        case Direction::U : 
            dog->Move(SpeedUnit{0., -speed});
            break;
