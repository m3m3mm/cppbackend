...
class ApiHandler {
public:
    ApiHandler(Strand api_strand) : api_strand_(api_strand) {
    }
    
    StringResponse HanldeApiRequest(const StringRequest& req, targets_storage::TargetRequestType req_type, const model::Game& game);
    Strand GetApiStrand();
private:
    Strand api_strand_;
            ...
            switch (req_type) {
                ...
                default : {
                    auto handle = [self = shared_from_this(), send,
                        ...
                        return send(self->api_handler_.HanldeApiRequest(req, req_type, self->game_));
                    };
