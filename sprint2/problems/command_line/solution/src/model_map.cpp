...
std::pair<Position, Speed> Map::CalculatePositionAndSpeedOnRoad(
    const Road &current_road, 
    const RoadBoarders &boarders, 
    const Position &calculated_pos, 
...
Position Map::GetRandomPosition() const {
    ...
    static std::uniform_int_distribution<> road_dist(0, roads_.size() - 1);
    static std::uniform_real_distribution<> pos_dist(0.0, 1.0);

    int road_id = road_dist(gen);
    auto boarders = road_boarders_[road_id];
    
    double random = pos_dist(gen);
