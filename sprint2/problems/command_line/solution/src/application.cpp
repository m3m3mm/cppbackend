...
ResponseInfo Application::MoveUnits(const Header& header, const std::string& req_body) const {
    if(auto content = FindHeader(header, CONTENT_TYPE); !content.has_value()
                                                        && *content != ContentType::APPLICATION_JSON) {
        return {http::status::bad_request,
                MakeBodyErrorJSON(TargetErrorCode::ERROR_INVALID_ARGUMENT_CODE,
...
void Application::MoveUnits(const std::chrono::milliseconds& delta) const {
