...
ApiHandler::ApiHandler(Strand&& api_strand, model::Game&& game, std::chrono::milliseconds&& timer) 
    : api_strand_(std::forward<Strand>(api_strand))
    , app_(std::move(game)) {

    if(timer.count() != 0 ) {
        ticker_ = std::make_shared<Ticker>(api_strand_, 
                                           std::forward<std::chrono::milliseconds>(timer), [&](std::chrono::milliseconds delta) {
                                           app_.MoveUnits(delta);
                                           });
        ticker_->Start(); 
    }
}
...
StringResponse ApiHandler::HanldeApiRequest(const StringRequest& req, TargetRequestType req_type) {
    ...
    switch (req_type) {
        ...
        case TargetRequestType::GET_MAP_BY_ID : {
            ...
            resp_info = std::make_unique<ResponseInfo>(app_.GetStaticObjectsInfo(req_type, req_obj));
            break;
        }
        ...
        case TargetRequestType::POST_TICK : {
            if(ticker_.get() == nullptr) {
                resp_info = std::make_unique<ResponseInfo>(app_.MoveUnits(req.base(), req.body()));
                break;
            }
        }
        ...
        default : {
            ...
            response.body() = MakeBodyJSON(req_type);
            break;
        }
    }
...
RequestHandler::RequestHandler(Strand&& api_strand, model::Game&& game, 
                               fs::path&& base_path, std::chrono::milliseconds&& timer)
    : base_path_(fs::weakly_canonical(base_path))
    , api_handler_(std::move(api_strand), std::move(game), std::move(timer)) {
}
